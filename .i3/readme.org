...
bindsym $mod+Shist+l --no-startup-id ~/.local/bin/byebye-exe
...
exec --no-startup-id feh --bg-scale ~/.config/vaporware.jpg
...
bar {
	status_command /home/ken/.config/bumblebee-status/bumblebee-status -m cpu memory redshift battery datetime \
